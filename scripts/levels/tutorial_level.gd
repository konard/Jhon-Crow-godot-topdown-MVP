...
##    - For shotgun: RMB UP (open bolt) → MMB DOWN (load shells) → RMB DOWN (close bolt)
	...
	match _current_step:
		...
		TutorialStep.RELOAD:
			if _has_shotgun:
				...
				# RMB drag UP (open bolt) → MMB drag DOWN (load shells, up to 8) → RMB drag DOWN (close bolt)
				_prompt_label.text = "[ПКМ↑ открыть] [СКМ↓ x8] [ПКМ↓ закрыть]"
			else:
